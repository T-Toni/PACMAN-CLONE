...
class aEstrelaBorda():
    ...
    def getCost(self, node):
        ...
        if node.col < len(self.node) / 2:
            xDistance = abs(node.col)
        else:
            xDistance = abs(node.col - len(self.node[0]))

        if node.row < len(self.node[0]) / 2:
            yDistance = abs(node.row)
        else:
            yDistance = abs(node.row - len(self.node[0]))
